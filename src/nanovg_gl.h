...
// Create a map from placeholder strings to enum values
std::map<std::string, GLNVGshaderType> placeholderEnumMap = {
        {"@NSVG_SHADER_FILLGRAD@", NSVG_SHADER_FILLGRAD},
        {"@NSVG_SHADER_FILLIMG@", NSVG_SHADER_FILLIMG},
        {"@NSVG_SHADER_SIMPLE@", NSVG_SHADER_SIMPLE},
        {"@NSVG_SHADER_IMG@", NSVG_SHADER_IMG},
        {"@NSVG_SHADER_DOTS@", NSVG_SHADER_DOTS},
        {"@NSVG_SHADER_FAST_ROUNDEDRECT@", NSVG_SHADER_FAST_ROUNDEDRECT},
        {"@NSVG_SHADER_FILLCOLOR@", NSVG_SHADER_FILLCOLOR},
        {"@NSVG_DOUBLE_STROKE@", NSVG_DOUBLE_STROKE},
        {"@NSVG_SMOOTH_GLOW@", NSVG_SMOOTH_GLOW},
        {"@NSVG_DOUBLE_STROKE_GRAD@", NSVG_DOUBLE_STROKE_GRAD},
        {"@NSVG_DOUBLE_STROKE_ACTIVITY@", NSVG_DOUBLE_STROKE_ACTIVITY},
        {"@NSVG_DOUBLE_STROKE_GRAD_ACTIVITY@", NSVG_DOUBLE_STROKE_GRAD_ACTIVITY}
};
...
static int glnvg__renderCreate(void* uptr)
{
    ...
    // replace placeholder @thing@ with NSVG_ enum value
    auto replaceAllOccurrences = [](std::string& str, const std::map<std::string, GLNVGshaderType>& placeholderEnumMap) {
        for (const auto& pair : placeholderEnumMap) {
            const std::string& placeholder = pair.first;
            std::string replacement = std::to_string(static_cast<int>(pair.second));
    ...
    // replace all placeholders inside the shader, so we can use enums without having to keep track of their raw enum number
    replaceAllOccurrences(fillFragShader, placeholderEnumMap);

    if (gl->flags & NVG_ANTIALIAS) {
		if (glnvg__createShader(&gl->shader, "shader", shaderHeader, "#define EDGE_AA 1\n", fillVertShader, fillFragShader.c_str()) == 0)
			return 0;
	} else {
		if (glnvg__createShader(&gl->shader, "shader", shaderHeader, NULL, fillVertShader, fillFragShader.c_str()) == 0)
			return 0;
	}
